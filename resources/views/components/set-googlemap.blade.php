...
<script>
    function initMap() {
        var token = "AIzaSyCGYnCh2Uusd7iASDhsUCxvbFgkSifkkTM";
        var oldlat = $('.old_lat').val();
        var oldlng = $('.old_long').val();
        console.log(1);
        if (oldlat) {
            oldlat = Number(oldlat);
            // alert(oldlat)
        } else {
            oldlat = {{  Session::has('location') ? Session::get('location')['lat'] : setting('default_lat') }};
        }
        if (oldlng) {
            oldlng = Number(oldlng);
        } else {
            oldlng = {{ Session::has('location') ? Session::get('location')['lng'] : setting('default_long') }};
        }
        // alert(oldlat);
        ...
        autocomplete.addListener('place_changed', function() {
            ...
            var form = new FormData();
            form.append('address', place.formatted_address);
            form.append('url', place.url);
            form.append('lat', place.geometry.location.lat());
            form.append('lng', place.geometry.location.lng());
